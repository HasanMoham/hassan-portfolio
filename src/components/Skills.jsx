...
import {
  SiJavascript, SiNetlify, SiVercel,
} from 'react-icons/si'
import { MdDevices } from 'react-icons/md'
import { TbApi, TbPlugConnected } from 'react-icons/tb'

const RestApiIcon = ({ size = 24, style }) => (
  <svg
...
const subsections = [
  {
    title: 'Frontend',
    skills: [
      { name: 'HTML5', icon: <FaHtml5 size={24} color="#E34F26" />, color: '#E34F26' },
      { name: 'CSS3', icon: <FaCss3Alt size={24} color="#1572B6" />, color: '#1572B6' },
      {
        name: 'JavaScript (ES6+)',
        icon: (
          <span
          ...
          >
            <SiJavascript size={16} color="#000" />
          </span>
        ),
        color: '#F7DF1E',
      },
      {
        ...
        icon: <MdDevices size={24} style={{ color: 'var(--accent)' }} />,
        color: null,
      },
    ],
  },
  {
    ...
    skills: [
      { name: 'React.js', icon: <FaReact size={24} color="#61DAFB" />, color: '#61DAFB' },
    ],
  },
  {
    ...
    skills: [
      {
        name: 'RESTful APIs',
        icon: <RestApiIcon size={24} style={{ color: 'var(--accent)' }} />,
        color: null,
      },
      {
        name: 'API Integration',
        icon: <TbPlugConnected size={24} style={{ color: 'var(--accent)' }} />,
        color: null,
      },
    ],
  },
  {
    ...
    skills: [
      { name: 'Netlify', icon: <SiNetlify size={24} color="#00C7B7" />, color: '#00C7B7' },
      {
        name: 'Vercel',
        icon: <SiVercel size={24} style={{ color: 'var(--text-primary)' }} />,
        color: null,
      },
    ],
  },
...
function SkillChip({ name, icon }) {
  return (
    <motion.div
      ...
      className="flex items-center gap-2 px-3 py-2 rounded-lg border cursor-default select-none text-sm 2xl:text-base"
      style={{
...
function SubsectionCard({ title, skills }) {
  return (
    <motion.div
      variants={cardVariants}
      className="rounded-xl border p-5 2xl:p-7"
      style={{
    ...
    >
      <h3
        className="font-semibold text-base 2xl:text-lg mb-4"
        style={{ fontFamily: 'Outfit, sans-serif', color: 'var(--accent)' }}
      >
        {title}
      </h3>
      <motion.div
        className="flex flex-wrap gap-2 2xl:gap-3"
  ...
  return (
    <div className="py-20 2xl:py-28" style={{ backgroundColor: 'var(--bg)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 2x2 Grid */}
        <motion.div
          className="grid grid-cols-1 sm:grid-cols-2 gap-5 2xl:gap-7"
        ...
        >
          {subsections.map((sub) => (
            <SubsectionCard key={sub.title} title={sub.title} skills={sub.skills} />
          ))}
        </motion.div>
